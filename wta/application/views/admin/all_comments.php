  <div id="main-content"> <!-- Main Content Section with everything --> 
    ...
    <h2>Comments</h2>
    <p id="page-intro"><?=$comments_num?> Comments</p>
...
$(function() {
	...
	$('.filter').change(function(){
		if($(this).attr('rel') == 1){
			window.location.href = '<?=base_url()?>admin/comments/select/'+$(this).val() +'/'+ $('#filter_users2').val();
		}else if($(this).attr('rel') == 2) {
		    window.location.href = '<?=base_url()?>admin/comments/select/'+ $('#filter_users1').val()+'/'+ $(this).val();
		}
	 ...
	 $('#applyForm').click(function(e){
		 ...
		 if($('#applyDrop').val() == 'delete'){
		    $("#applySelected").attr("action", "<?=base_url()?>admin/comment_mdelete/n");
