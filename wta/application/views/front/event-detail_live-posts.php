											...
											if(obj){
												...
												$(obj['atts']).each(function(i, d){	
													...
													if((d.facebook_id == 0)&&(d.twitter_id == 0)){
														...
														html +='<img src="https://graph.facebook.com/'+d.username+'/picture" rel="'+d.id+'" >';
												...
												<?php elseif($this->facebook_admin):?>
												      htmlDeliver +='<img src="https://graph.facebook.com/<?=$this->session->userdata('facebook_username')?>/picture" rel="<?=$user_id?>" />';
					...
					$('#give_comment').submit(function(e){
						...
						$("input[type=submit]").attr("disabled", true);
							...
							if(is_blocked == 1){
								...
								$('#give_comment textarea').val('');
									...
									userImg ='<img src="/phpThumb/phpThumb.php?src=/assets/users_img/<?=$this->session->userdata('img')?>&amp;w=57&amp;h=57&amp;zc=1&amp;aoe=1&amp;fltr[]=wmt||15|C|FFFFFF|arial.ttf|100|100||000000|100|x" />';
									<?php elseif($this->facebook_admin):?>
									userName = '<?=$this->session->userdata('facebook_username')?>';
									userImg ='<img src="https://graph.facebook.com/<?=$this->session->userdata('facebook_username')?>/picture" />';
									<?php elseif($this->twitter_admin):?>
								   ...
								   $.ajax({
											...
											success: function(obj){
												...
												if(obj){
													...
													var comment_send = JSON.stringify({"userImg": userImgApe , "comment": commentApe , "userName": userNameApe , "admin": admin, "comments_id": scope_comments_id_ape });	
						...
						*/
						...
						<?php if(isset($post_comments)):?>
									...
									userImg_local ='<img src="/phpThumb/phpThumb.php?src=/assets/users_img/<?=$this->session->userdata('img')?>&amp;w=57&amp;h=57&amp;zc=1&amp;aoe=1&amp;fltr[]=wmt||15|C|FFFFFF|arial.ttf|100|100||000000|100|x" />';
									<?php elseif($this->facebook_admin):?>
									userName_local = '<?=$this->session->userdata('facebook_username')?>';
									userImg_local ='<img src="https://graph.facebook.com/<?=$this->session->userdata('facebook_username')?>/picture" />';
									<?php elseif($this->twitter_admin):?>
								   ...
								   $.ajax({
											...
											success: function(obj){
												...
												if(obj){
                                                    ...
                                                    var comment_send = JSON.stringify({"userImg": userImgApe , "comment": commentApe , "userName": userNameApe , "admin": admin_local, "comments_id": scope_comments_id_ape });													
													pipe.send(comment_send);
						...
						<?php endif; ?>
						   ...
						   function checkLastIdSend(inserted_id, admin_local, comment_local,userName_local, userImg_local, soc_post){
						   
							   var scope_comments_id_ape = inserted_id;
							   var last_div_id = parseInt($("#push_comments li:first").attr("data-id"));
							   var last_div_id_incrementedByOne = last_div_id + 1;
					...
					<div class="five columns alpha omega people" id="thumb_att">
                      <?php foreach($attendings as $attending): ?>
                          ...
                          <?php elseif($attending['facebook_id']!=0):?>
                                <img src="https://graph.facebook.com/<?=$attending['username']?>/picture" rel="<?=$attending['id']?>" />
...
function printHTML(obj, imparity, lastID){
				...
				$(obj).each(function(i, d){	
						...
						if(d.facebook_id != 0){
						 comment_ajax += 'https://graph.facebook.com/'+d.username+'/picture';
						} else if(d.twitter_id != 0){ 
						 comment_ajax += d.img;
						}else{
