...
function release_ape(){ 		
     var client = new APE.Client;
                ...
                'complete': function(ape){
				  ...
				  var scope_comments_id = <?=$initial_comments_id?>;
				  ...
				  ape.addEvent('multiPipeCreate', function(pipe, options) {
							...
							function like(comment_id, change_span){
										...
										success: function(obj){
											if(obj['result'] == 1){
												...
												pipe.send(apeLike);
												/*alert('You successfully liked comment!');*/
											}else{
										...
										success: function(obj){
											...
											if(obj){
												...
												<?php if($this->logged_in):?>
												      htmlDeliver +='<img src="http://wall2allnew.cp-dev.com/phpThumb/phpThumb.php?src=/assets/users_img/<?=$this->session->userdata('img')?>&amp;w=57&amp;h=57&amp;zc=1&amp;aoe=1&amp;fltr[]=wmt||15|C|FFFFFF|arial.ttf|100|100||000000|100|x" rel="<?=$user_id?>" />';
					...
					$('#give_comment').submit(function(e){
						...
						if($('input[name="userfile"]').val()===''){
							...
							if(is_blocked == 1){
								...
								$('#give_comment textarea').val('');
									...
									userImg ='<img src="http://wall2allnew.cp-dev.com/phpThumb/phpThumb.php?src=/assets/users_img/<?=$this->session->userdata('img')?>&amp;w=57&amp;h=57&amp;zc=1&amp;aoe=1&amp;fltr[]=wmt||15|C|FFFFFF|arial.ttf|100|100||000000|100|x" />';
									<?php elseif($this->facebook_admin):?>
									userName = '<?=$this->session->userdata('facebook_Fname').' '.$this->session->userdata('facebook_Lname')?> ';
									userImg ='<img src="http://graph.facebook.com/<?=$this->session->userdata('facebook_username')?>/picture" />';
									<?php elseif($this->twitter_admin):?>
									userImg = '<?=$this->session->userdata('twitter_username')?>';
								   ...
								   $.ajax({
											...
											success: function(obj){
												...
												if(obj['result'] == 1){
													...
													scope_comments_id++;
													var comment_add = '<li class="ten columns alpha omega post'+admin+' delete"><div class="bouble"><h3>'+userName+'</h3>'+comment;
													comment_add += '<span class="meta">'+currentTime()+ '| <a href="" class="user_like" rel="'+scope_comments_id+'"> Like </a> | <a href="" class="user_report" rel="'+scope_comments_id+'"> Report </a>';
													comment_add +='<a href="" class="user_comm_delete" rel="'+scope_comments_id+'"> | Delete</a>'
						...
						<?php if(isset($post_comments)):?>
									...
									userImg_local ='<img src="http://wall2allnew.cp-dev.com/phpThumb/phpThumb.php?src=/assets/users_img/<?=$this->session->userdata('img')?>&amp;w=57&amp;h=57&amp;zc=1&amp;aoe=1&amp;fltr[]=wmt||15|C|FFFFFF|arial.ttf|100|100||000000|100|x" />';
									<?php elseif($this->facebook_admin):?>
									userName_local = '<?=$this->session->userdata('facebook_Fname').' '.$this->session->userdata('facebook_Lname')?> ';
								   ...
								   $.ajax({
											...
											success: function(obj){
												...
												if(obj['result'] == 1){
													...
													var comment_add = '<li class="ten columns alpha omega post'+admin_local+' delete"><div class="bouble"><h3>'+userName_local+'</h3>'+comment_local;
													comment_add += '<span class="meta">'+currentTime()+ '| <a href="" class="user_like" rel="'+scope_comments_id+'"> Like </a> | <a href="" class="user_report" rel="'+scope_comments_id+'"> Report </a>';
													comment_add +='<a href="" class="user_comm_delete" rel="'+scope_comments_id+'"> | Delete</a>'
													comment_add += soc_post +'</span></div>'
						...
						if(raw.data.msg !== undefined){
							...
							var outputM = jQuery.parseJSON(decodeURIComponent(raw.data.msg));
							   ...
							   if(outputM.deletePost !== undefined){ 
								     ...
								     $(".user_like[rel='"+ outputM.deletePost +"']").parents('li .delete').remove();
								    ...
								    var comment_caught= '<li class="ten columns alpha omega post'+outputM.admin+' delete"><div class="bouble"><h3>'+ outputM.userName.replace(/~/g,'"')+'</h3>'+outputM.comment.replace(/~/g,'"');
									...
									comment_caught += outputM.userImg.replace(/~/g,'"') +'</li>';
...
<script>
  window.fbAsyncInit = function() {
    FB.init({
      appId      : '326283710790999', // App ID
      channelUrl : 'http://wall2allnew.cp-dev.com/channel.html', // Channel File
						  	...
						  	<ul class="ten columns alpha omega posts">
                                ...
                                <?php if(($logged_in)||($face_logged_in)||($twit_logged_in)): ?>
                                    <!--<form id="give_comment" action=""  method="post">-->
                                     ...
                                     echo form_open_multipart(base_url().'events/detail/'.$event->id_events, $attr);
...
<script>
document.domain = "wall2allnew.cp-dev.com";
...
function ajax_comments(initOffset,initTime){
	
	
	        if(initOffset === false){
				var offset = parseInt($('.load_more_ajax a').attr('rel')) + 15;
			}else{
				var offset = 0;
			}
			...
			success: function(obj){
               ...
               if(obj['result'] == 1){
				...
				$(obj['comments']).each(function(i, d){	
						 ...
						 comment_ajax += '<li class="ten columns alpha omega post'+ admin_is +' delete"><div class="bouble"><h3>'+ d.username +'</h3>'+d.comment;
					    ...
					    $('.load_more_ajax a').attr('rel',offset);
